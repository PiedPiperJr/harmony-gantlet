...
class ModelConfig:
    ...
    n_samples_per_letter: int = 500      # Number of samples per letter
    n_snapshots: int = 50                # Number of snapshots per sample
    n_variables: int = 11               # Number of variables (5 flex + 6 accelerations)
    normal_dist_range: Tuple[float, float] = (-3, 3)  # Range for normal distribution
...
class LetterPredictor:
        ...
        for k in range(self.config.n_samples_per_letter):
            ...
            with open(file_path, 'w', newline='') as f:
                ...
                for _ in range(self.config.n_snapshots):
                    row = np.concatenate([
                        np.random.normal(450, 100, size=4).astype(int).clip(100, 800),
                        np.random.uniform(100, 800, size=4).astype(int),
                        np.random.exponential(scale=200, size=3).astype(int).clip(100, 800)
                    ])
                    writer.writerow(row)
        ...
        try:
            for letter in ascii_lowercase:
                for k in range(self.config.n_snapshots):
                    sample_path = Path(base_dir) / f"position_{k}" / f"{letter}.csv"
                    self.data[letter][k] = np.loadtxt(sample_path, delimiter=',')
    ...
    def compute_mean_states(self, use_normal_law: bool = True) -> None:
    ...
    def compute_mean_positions(self, use_normal_law: bool = True) -> None:
        ...
        x = np.linspace(self.config.normal_dist_range[0], 
                       self.config.normal_dist_range[1], 
                       self.data[ascii_lowercase[0]].shape[1])
        weights = norm.pdf(x, loc=0, scale=1)
        weights /= np.sum(weights)
    ...
    def predict_v1(self, input_data: np.ndarray) -> str:
        ...
        if self.mean_states is None:
            raise ValueError("Model not trained. Call compute_mean_states first.")
    ...
    def predict_v2(self, input_data: np.ndarray, 
        ...
        if self.mean_positions is None:
            raise ValueError("Model not trained. Call compute_mean_positions first.")
    ...
    def _process_position(self, position: int, source_dir: Path, target_dir: Path) -> None:
        ...
        for letter in ascii_lowercase:
            ...
            if letter_data:
                output_path = target_dir / f"position_{position}" / f"{letter}.csv"
                try:
